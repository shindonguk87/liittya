...
import { setEndDateAction } from '../../../../store/list/actions';
...
const _종료날짜_팝업 = () => {
  ...
  useEffect(() => {
    if (날짜데이터) {
      (window as any).$('#toContainer').empty();
      ...
      (window as any).$('#toContainer').append(div);
      ...
      div
        ...
        .on('monthpicker-change-year', function (e, year) {
          restricMonth(saleYmdFrom, saleYmdTo, 'to', year);
        })
      ...
      (window as any).$('#to').on('change', function () {
        dispatch(
          setEndDateAction({
            endDate: (window as any).$('#to').val().split('-').toString().replaceAll(',', ''),
          })
        );
        (window as any).$('#to').val('To');
