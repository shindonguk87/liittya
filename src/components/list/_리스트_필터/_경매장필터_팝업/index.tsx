...
import { setAuctionTypeAction } from '../../../../store/list/actions';
...
const _경매장필터_팝업 = ({ 열림, 타이틀, 필터데이터, onClose }: Props) => {
  const dispatch = useDispatch();
  const { auctionType } = useSelector<RootState, State>(selectors.listState);
  const 선택데이터 = useMemo(() => (auctionType ? auctionType.split(',') : []), [auctionType]);

  const 경매장_변경 = useCallback(
    (key: string) => {
      if (선택데이터?.includes(key)) {
        if (auctionType) {
          const set = new Set(auctionType.split(','));
          set.delete(key);
          dispatch(
            setAuctionTypeAction({
              auctionType: Array.from(set).join(','),
            })
          );
        } else {
          dispatch(
        ...
        {Object.keys(필터데이터).map(key => {
          return (
            <_필터_아이템 key={key}>
              <_체크박스
                type="checkbox"
                checked={선택데이터 && 선택데이터.includes(key)}
                onChange={() => 경매장_변경(key)}
              />
              {필터데이터[key]}
            </_필터_아이템>
